...
def principal_feature_analysis(cluster_size, data, number_output_functions, freq_data, l, left_features, alpha, shuffle_feature_numbers):
    # Subtract the number of components of the output function
    number_nodes = len(left_features) - number_output_functions
    # Take only the features and not the components of the output function
    list_of_nodes = left_features[number_output_functions:].copy()
    ...
    # number of chi-square tests with a bin less than 5 data points
    counter_bin_less_than5 = 0
    # number of chi-square tests with a bin less than 1 data point
    counter_bin_less_than1 = 0
    ...
    while(True):
        ...
        if shuffle_feature_numbers == 1:
            # Pick the nodes of the subgraphs randomly if uncommented
            ...
            for j in range(0, cluster_size):
                if (i * cluster_size + j) < number_nodes:
                    intermediate_list.append(
                        list_of_nodes[i * cluster_size + j])
            if intermediate_list != []:
                list_of_clusters.append(sorted(intermediate_list))
                intermediate_list = []
        list_of_nodes = []

        # flag to notify if no further node was removed from any subgraph
        any_cluster_dissected = 0
        ...
        for cluster in list_of_clusters:
            ...
            for i in range(0, number_elements_cluster):
                for j in range(i + 1, number_elements_cluster):
                    ...
                    if is_entry_calculated[cluster[i], cluster[j]] == 0 and (len(freq_data[cluster[i]]) > 1 and len(freq_data[cluster[j]]) > 1):
                        ...
                        expfreq = np.outer(
                            freq_data[cluster[i]], freq_data[cluster[j]]) / n
                        ...
                        pv = scipy.stats.chisquare(
                            freq_data_product.flatten(), expfreq.flatten(), ddof=-1)[1]
            ...
            # add the entries of the adjacency matrix of the subcluster to the global adjacency matrix
            adjm = (global_adjm[cluster, :])[:, cluster]
            ...
            # list of lists of nodes corresponding to the complete subgraphs of list_complete_sub_graphs
            list_nodes_complete_sub_graphs = []
            ...
            while list_graphs_to_divide != []:
                ...
                for current_graph in list_graphs_to_divide:
                    ...
                    for node in list(set_nodes_to_delete):
                        # remove the nodes that were found with the minimum cut algorithm
                        current_graph.remove_node(node)
                    list_new_sub_graphs = [current_graph.subgraph(
                        c).copy() for c in nx.connected_components(current_graph)]
                    ...
                    for sub_graph_of_current_graph in list_new_sub_graphs:
                        if list(nx.complement(sub_graph_of_current_graph).edges) != []:
                            list_graphs_to_divide.append(
                                sub_graph_of_current_graph)
                        else:
                            list_complete_sub_graphs.append(
                                sub_graph_of_current_graph)
                            list_nodes_complete_sub_graphs.append(
                                list(sub_graph_of_current_graph.nodes))
        ...
        list_of_nodes.sort()
        # Update the number of left features
        number_nodes = len(list_of_nodes)
