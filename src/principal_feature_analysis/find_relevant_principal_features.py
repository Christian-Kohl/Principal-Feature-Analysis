...
def find_relevant_principal_features(data, number_output_functions, cluster_size, alpha, min_n_datapoints_a_bin, shuffle_feature_numbers, frac):
    ...
    list_principal_features, smaller_than5, smaller_than1 = principal_feature_analysis(
        cluster_size, data, number_output_functions, freq_data, l, left_features, alpha, shuffle_feature_numbers)
    ...
    for i in list_principal_features:
        intermediate_list = []
        for j in i:
            # Output functions, which are not considered in the PFA, are the values 0,1,...,number_output_functions-1 in left_features
            shift_variable = j + number_output_functions
